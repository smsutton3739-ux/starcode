...
class AstronomicalEvent(UUIDPrimaryKey, Timestamped, Base):
    ...
    *computed* (produced by the ephemeris engine and cached). `is_computed` keeps the two
    ...
    # 40, not 20: the default value below is itself 26 characters, so every row that did
    # not set this column explicitly — every comet and supernova in the catalogue —
    # overflowed it. SQLite ignores VARCHAR lengths, so the whole test suite passed while
    # seeding a real PostgreSQL failed outright.
    calendar_used: Mapped[str] = mapped_column(String(40), default="julian_gregorian_proleptic")
